...
# Функция для создания сводной таблицы всех экспериментов
def create_leaderboard():
    try:
        # Поиск всех запусков в эксперименте :cite[2]:cite[8]
        runs = mlflow.search_runs(search_all_experiments=True)
        
        if len(runs) == 0:
            print("⚠️ Не найдено запусков для создания leaderboard")
            return None
        
        # Выбираем только нужные колонки
        leaderboard_columns = [
            'run_id', 'experiment_name', 'status', 
            'metrics.accuracy', 'metrics.macro_f1', 'metrics.weighted_f1',
            'params.model', 'params.scaler', 'params.feature_variant',
            'params.validation_scheme', 'start_time'
        ]
        
        # Фильтруем только существующие колонки
        available_columns = [col for col in leaderboard_columns if col in runs.columns]
        leaderboard_df = runs[available_columns].copy()
        ...
        column_mapping = {
            ...
            'params.feature_variant': 'feature_variant',
            'params.validation_scheme': 'validation_scheme'
        }
        
        leaderboard_df.rename(columns=column_mapping, inplace=True)
        
        # Сортируем по accuracy (по убыванию)
        if 'accuracy' in leaderboard_df.columns:
            leaderboard_df = leaderboard_df.sort_values('accuracy', ascending=False)
        
        # Сохраняем в CSV
        leaderboard_path = "leaderboard.csv"
        leaderboard_df.to_csv(leaderboard_path, index=False, encoding='utf-8')
        
        print(f"✅ Leaderboard создан с {len(leaderboard_df)} запусками")
        return leaderboard_path
        
    except Exception as e:
        print(f"❌ Ошибка при создании leaderboard: {e}")
        return None
...
def train_model(args):
    ...
    with mlflow.start_run(run_name=f"{model_name}_{feature_variant}_{scaler_name}"):
        ...
        # Создание и логирование визуализаций через mlflow.log_figure() :cite[1]:cite[7]
        ...
        try:
            ...
            ax.grid(True, alpha=0.3)
            plt.tight_layout()
            mlflow.log_figure(fig_perm, "plots/permutation_importance.png")
        except Exception as e:
...
def log_leaderboard():
    """Создает и логирует сводную таблицу всех экспериментов"""
    leaderboard_path = create_leaderboard()
    if leaderboard_path:
        mlflow.log_artifact(leaderboard_path, "leaderboard")
        print(f"Leaderboard сохранен как артефакт: {leaderboard_path}")
         # Чтение и вывод топ-5 результатов
    leaderboard_df = pd.read_csv(leaderboard_path)
    if len(leaderboard_df) > 0:
        print("\nТоп-5 моделей по accuracy:")
        print(leaderboard_df.head(5).to_string(index=False))
    else:
        print("Не удалось создать leaderboard")
...
if __name__ == "__main__":
    ...
    # Создаем и логируем leaderboard
    log_leaderboard()
